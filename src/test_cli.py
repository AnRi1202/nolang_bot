...
class RAGSystem:
    ...
    def search(self, query: str, k: int = 5) -> list[dict]:
        ...
        try:
            ...
            distances, indices = self.model.kneighbors(
                query_vector, n_neighbors=min(k, len(self.metadata))
            )
    ...
    def generate_response(
        self, question: str, contexts: list[dict], user_email: str = None, inquiry_type: str = None
    ) -> dict:
        """回答とメール文を生成"""
        if not contexts:
            return {
        ...
        answer = self._generate_answer(
            question, contexts, similar_cases, contact_info, user_email, inquiry_type
        )

        # メール文を生成
        email_draft = self._generate_email_draft(
            question, contexts, similar_cases, contact_info, user_email, inquiry_type
        )
...
### 回答"""
    ...
    def _generate_email_draft(
        ...
        if similar_cases:
            ...
            for case in similar_cases[:2]:
                past_cases_summary += (
                    f"- {case['question'][:50]}... (状況: {case['status']}, 日付: {case['date']})\n"
                )
